...
/////////////////////////////////
class paddle_h_list_element : wall_list_element
{
public:


 //===============================================================
	paddle_h_list_element(  const int i_xpos, 
							const int i_ypos, 
							LPDIRECTDRAWSURFACE7 &p_dds,
							double OwningPlayerID) : wall_list_element (i_xpos, i_ypos, p_dds)
{
	i_w_wall	=	60; // won't show if bigger then image 		  
	i_h_wall	=	16; // won't show if bigger then image        
	i_wait      =	0;
	...
	void move(wall_list_element *first_ptr, wall_list_element *cur_element, CPlayer &Player1) 
{
	 if (Player1.i_my_slot == cur_element->OwningPlayerID)
		{	// my paddle move (left/right)
			// my paddle move (left/right)
			// my paddle move (left/right)
			 cur_element->i_xpos_wall += Player1.i_x_pos[Player1.i_my_slot];
			//========================
			if ( cur_element->i_xpos_wall < 0 +80 )
				cur_element->i_xpos_wall = 0+80;

			if ( cur_element->i_xpos_wall + cur_element->i_w_wall >592 -80)
				cur_element->i_xpos_wall = 592 -80- cur_element->i_w_wall;
...
//			TRACE( TEXT("My Paddle  x= %i, y= %i , p= %i\n"), cur_element->i_xpos_wall, cur_element->i_ypos_wall,cur_element->OwningPlayerID );
		...
		if (Player1.i_PlayerType[cur_element->OwningPlayerID] == Computer)
			{	// this is the computer moving (AI)
				...
				int move_rate =15;
				//int xxx;
				//xxx = (int)cur_element->i_xpos_wall;
			//TRACE( TEXT("Paddle  AI1= %i,AI2= %i, objID= %i , Player= %i, X = %i, xwal = %i\n"), cur_element->AI_X1,cur_element->AI_X2, cur_element->i_object_ID,cur_element->OwningPlayerID,cur_element->Return_X(), xxx);
				if (cur_element->i_ypos_wall < 296)  // 592/2
						// left paddle (up/down)
							temp_move=((double)(592-cur_element->AI_Y)/592)*move_rate;
					else
						// right paddle (up/down)
							temp_move=((double)(cur_element->AI_Y)/592)*move_rate;
				// calculate move left or right based on ball pos
				if (cur_element->AI_X > cur_element->i_xpos_wall + (cur_element->i_w_wall/2) +(move_rate))
						cur_element->i_xpos_wall +=temp_move;
					else
						if (cur_element->AI_X < cur_element->i_xpos_wall + (cur_element->i_w_wall/2)-(move_rate))
							cur_element->i_xpos_wall -=temp_move;
				// must still play by the rules, stay in bounds

//cur_element->i_xpos_wall=cur_element->AI_X-22; /// EXACTLY WHERE BALL IS, NOW
			...
			{	// this someone else paddle
				// this someone else paddle
				// this someone else paddle
//			if (Player1.i_x_1_pos[cur_element->OwningPlayerID] != 0)
//						cur_element->i_xpos_wall = Player1.i_x_1_pos[cur_element->OwningPlayerID];
//				if (Player1.i_y_1_pos[cur_element->OwningPlayerID] != 0)
//						cur_element->i_ypos_wall = Player1.i_y_1_pos[cur_element->OwningPlayerID];
