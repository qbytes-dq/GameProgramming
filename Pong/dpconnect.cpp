...
HWND g_hDlg_connect1;		//DQ005
HWND g_hDlg_connect2;		//DQ005
HWND g_hDlg_connect3;		//DQ005
HWND g_hDlg_connect4;		//DQ005
HWND g_hDlg_connect5;		//DQ005
//-----------------------------------------------------------------------------
// Function-prototypes
//-----------------------------------------------------------------------------
...
LRESULT CALLBACK DlgProcEnumPlayers( HWND );
...
int DPConnect_StartDirectPlayConnect( HINSTANCE hInst, BOOL bBackTrack = FALSE )
{
    ...
    while( TRUE )
    {
	OutputDebugString ("i'm here - WM_INITDIALOG\n");

        switch( nStep )
        {
            case 0:
				OutputDebugString ("DPConnect_StartDirectPlayConnect nStep = 0 \n");
            ...
            case 1:
			OutputDebugString ("DPConnect_StartDirectPlayConnect nStep = 1 \n");
...
{
    HRESULT hr;
	g_hDlg_connect1 = hDlg;	//dq005
    switch( msg )
    {
        case WM_INITDIALOG:
            {
			OutputDebugString ("DPConnect_ConnectionsDlgProc WM_INITDIALOG\n");
        ...
        case WM_COMMAND:
			OutputDebugString ("DPConnect_ConnectionsDlgProc WM_COMMAND - ");

            switch( LOWORD(wParam) )
            {
                case IDC_CONNECTION_LIST:
			OutputDebugString ("IDC_CONNECTION_LIST ");
                ...
                case IDOK:
                    ...
                    if( FAILED( hr = DPConnect_ConnectionsDlgOnOK( hDlg ) ) )
                        EndDialog( hDlg, EXITCODE_ERROR );		//DQ005 need to verify this is closed
																//DQ005 why is it being closed only on a problem
        ...
        case WM_DESTROY:
			OutputDebugString ("DPConnect_ConnectionsDlgProc WM_DESTROY\n");
...
HRESULT DPConnect_ConnectionsDlgFillListBox( HWND hDlg )
{
    ...
    int           nIndex;
OutputDebugString ("DPConnect_ConnectionsDlgFillListBox \n");
                                                   ...
                                                   VOID*     pvContext )
{
    ...
    LRESULT       iIndex;
OutputDebugString ("DPConnect_EnumConnectionsCallback \n");
...
HRESULT DPConnect_ConnectionsDlgOnOK( HWND hDlg )
{
    ...
    LRESULT iIndex;
    HRESULT hr;
	OutputDebugString ("DPConnect_ConnectionsDlgOnOk \n");
...
VOID DPConnect_ConnectionsDlgCleanup( HWND hDlg )
{
    ...
    DWORD   iIndex;
    DWORD   dwCount;
OutputDebugString ("DPConnect_ConnectionsDlgCleanup \n");
...
LRESULT CALLBACK DlgProcEnumPlayers( HWND hWnd )
{
	int iIndex;
    LPGUID lpGuid;

	HWND          hWndListBox1 = GetDlgItem( hWnd, IDC_GAMES_LIST );
	HWND          hWndListBox2 = GetDlgItem( hWnd, IDC_PLAYERS );
	...
	//OutputDebugString ("DPConnect_dlg proc enum players- WM_INITDIALOG\n");
	iIndex = SendDlgItemMessage (hWnd,IDC_GAMES_LIST, LB_GETCURSEL, 0, 0L );

    lpGuid = (LPGUID) SendMessage( hWndListBox1, LB_GETITEMDATA,  iIndex, 0 );

    if FAILED( g_pDP->EnumPlayers( lpGuid, 
                    ( LPDPENUMPLAYERSCALLBACK2 )EnumPlayer,
                    ( LPVOID )GetDlgItem( hWnd, IDC_PLAYERS ),
                    DPENUMPLAYERS_SESSION ) )
			{
				SendMessage( hWndListBox2, LB_ADDSTRING, 0, (LPARAM)"No Games for players... " );
			}
...
BOOL CALLBACK DPConnect_SessionsDlgProc( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam )
{
    ...
    HRESULT hr;
	g_hDlg_connect2 = hDlg;	//dq005	

    switch( msg )
    {
        case WM_INITDIALOG:
            {
			OutputDebugString ("DPConnect_SessionsDlgProc - WM_INITDIALOG \n");
				...
				HWND	hWndListBox2 = GetDlgItem( hDlg, IDC_PLAYERS );
				SendMessage( hWndListBox2, LB_ADDSTRING, 0, (LPARAM)"No Games for players... " );


				// set default radio buttons
				CheckDlgButton( hDlg, IDC_FULL_SCREEN, BST_CHECKED );
				CheckDlgButton( hDlg, IDC_800, BST_CHECKED );
				
				// disable because only valid in a window
				EnableWindow( GetDlgItem( hDlg, IDC_800 ), false );
				EnableWindow( GetDlgItem( hDlg, IDC_1024 ), false );
				EnableWindow( GetDlgItem( hDlg, IDC_1280 ), false );
        ...
        case WM_TIMER:
		OutputDebugString ("DPConnect_SessionsDlgProc - WM_TIMER \n");

            if( FAILED( hr = DPConnect_SessionsDlgShowGames( hDlg ) ) )
            {
                KillTimer( hDlg, 1 );
                MessageBox( hDlg, TEXT("Error enumerating DirectPlay games."),
                            TEXT("Q Bytes DirectPlay"), 
                            MB_OK | MB_ICONERROR );
            }
            break;

        case WM_COMMAND:
		OutputDebugString ("DPConnect_SessionsDlgProc - WM_COMMAND\n");
		//DlgProcEnumPlayers(hDlg);	//1
            switch( LOWORD(wParam) )
            {
				// when using radio buttons, be sure to put them in a group box, this 
				// will allow only that set to be changed when a button is pressed
				// i.e. uncheck the other item in that group.
				// alse set the first item to: Group = True (in properties window)

				case IDC_WINDOW:
					{
					CheckDlgButton( hDlg, IDC_WINDOW, BST_CHECKED );

					EnableWindow( GetDlgItem( hDlg, IDC_800 ), true );
					EnableWindow( GetDlgItem( hDlg, IDC_1024 ), true );
					EnableWindow( GetDlgItem( hDlg, IDC_1280 ), true );
				...
				case IDC_FULL_SCREEN:
					{
					//CheckDlgButton( hDlg, IDC_WINDOW, BST_UNCHECKED );
					//CheckDlgButton( hDlg, IDC_FULL_SCREEN, BST_UNCHECKED );
					CheckDlgButton( hDlg, IDC_FULL_SCREEN, BST_CHECKED );

					CheckDlgButton( hDlg, IDC_800, BST_CHECKED );
					CheckDlgButton( hDlg, IDC_1024, BST_UNCHECKED );
					CheckDlgButton( hDlg, IDC_1280, BST_UNCHECKED );
				...
				case IDC_1024:
					{
					CheckDlgButton( hDlg, IDC_1024, BST_CHECKED );
					g_Resolution = 1024;
					}
					break;
				...
				case IDC_1280:
					{
					CheckDlgButton( hDlg, IDC_1280, BST_CHECKED );
					g_Resolution = 1280;
					}
					break;
        ...
        case WM_DESTROY:
OutputDebugString ("DPConnect_SessionsDlgProc - WM_DESTROY\n");
...
VOID DPConnect_SessionsDlgInitListbox( HWND hDlg )
{
    ...
    LPDIRECTPLAY4 pDP = NULL;
OutputDebugString ("DPConnect_DlgInitListbox \n");
...
OutputDebugString ("DPConnection_SessionsDlgShowGames \n");
...
DlgProcEnumPlayers(hDlg);	// good location
...
OutputDebugString ("DPConnect_EnumSessionsCallback \n");
...
HRESULT DPConnect_SessionsDlgJoinGame( HWND hDlg )
{
    ...
    DPSessionInfo* pDPSISelected = NULL;
    int            nItemSelected;
OutputDebugString ("DPConnect_SessionsDlgJoinGame \n");
...
HRESULT DPConnect_SessionsDlgCreateGame( HWND hDlg )
{
    ...
    int            nResult; 
    HINSTANCE      hInst;
OutputDebugString ("DPConnect_SessionsDlgCreateGame \n");
...
{
    DWORD dwNameLength;
	g_hDlg_connect3 = hDlg;	//dq005	

    switch( msg ) 
    {
        case WM_INITDIALOG:
OutputDebugString ("DPConnect_CreateSessionsDlgProc - WM_INITDIALOG\n");
        ...
        case WM_COMMAND:
OutputDebugString ("DPConnect_CreateSessionsDlgProc - WM_COMMAND\n");
...
VOID DPConnect_SessionsDlgCleanup()
{
	OutputDebugString ("DPConnect_SessionsDlgCleanup \n");
...
OutputDebugString ("DPConnect_WaitForLobbyLaunch \n");
...
HRESULT DPConnect_CheckForLobbyLaunch( BOOL* pbLaunchedByLobby )
{
    HRESULT hr;
    DWORD   dwSize;
OutputDebugString ("DPConnect_CheckForLobbyLaunch \n");
...
{
    HRESULT hr;
OutputDebugString ("DPConnect_DoLobbyLaunch \n");
...
OutputDebugString ("DPConnect_lobbyWaitDlgProc \n");
...
HRESULT DPConnect_GetLobbyConnectionSettingsMessage()
{
    ...
    DWORD   dwMsgFlags;
OutputDebugString ("DPConnect_GetLobbyConnectsionsSettingsMessage \n");
